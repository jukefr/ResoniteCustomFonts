	...
	public static partial class CustomFontsPatches
	{
		private static IEnumerable<object> EnumerateParentSlots(object startSlot)
		{
			...
			while (current != null && guard < 128)
			{
				...
				current = ReadMemberValue(current, "Parent");
				guard++;
			}
		}
		...
		private static IEnumerable<object> EnumerateFontChainComponentsOnSlot(object slot)
		{
			...
			{
				if (comp != null && IsLikelyFontChain(comp.GetType()))
					yield return comp;
			}
		}
		...
		private static IEnumerable<object> EnumerateComponentsOnSlot(object slot)
		{
			...
			{
				var result = SafeRead(() => getComponentsNoArg.Invoke(slot, null));
			...
			{
				var fontChainType = AccessTools.TypeByName("FrooxEngine.FontChain");
				if (fontChainType != null)
				{
					var arr = SafeRead(() => getComponentsType.Invoke(slot, new object[] { fontChainType }));
					if (arr is System.Collections.IEnumerable e)
					{
						foreach (var c in e)
						{
							if (c != null)
				...
				var textType = AccessTools.TypeByName("FrooxEngine.UIX.Text");
				if (textType != null)
				{
					var arr = SafeRead(() => getComponentsType.Invoke(slot, new object[] { textType }));
					if (arr is System.Collections.IEnumerable e)
					{
						foreach (var c in e)
						{
			...
			{
				var list = ReadMemberValue(slot, fieldName);
		...
		private static object? GetComponentOnSlot(object slot, Type componentType)
		{
			...
			var getByType = slotType.GetMethods(flags).FirstOrDefault(m =>
				...
				&& m.GetParameters()[0].ParameterType == typeof(Type));
			if (getByType != null)
				return SafeRead(() => getByType.Invoke(slot, new object[] { componentType }));
		...
		private static object? GetComponentInParents(object slot, Type componentType)
		{
			...
			foreach (var m in slotType.GetMethods(BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance))
			{
				...
				try
				{
					...
					{
						var r = m.MakeGenericMethod(componentType).Invoke(slot, new object[] { false });
						if (r != null)
							return r;
						return m.MakeGenericMethod(componentType).Invoke(slot, new object[] { true });
					}
				}
				catch
				{
		...
		private static object? FindSceneInspectorByWalkingParentSlots(object slot, Type sceneInspectorType)
		{
			foreach (var s in EnumerateParentSlots(slot))
			{
				var c = GetComponentOnSlot(s, sceneInspectorType);
				if (c != null)
					return c;
			}
			return null;
		...
		/// <summary>
		/// <see cref="FrooxEngine.Slot.GetComponentsInChildren{T}()"/> via reflection (no compile-time FrooxEngine generic reference).
		/// </summary>
		private static IEnumerable<object> EnumerateComponentsInChildrenOfSlot(object rootSlot, Type componentType)
		{
			...
			var list = SafeRead(() => constructed.Invoke(rootSlot, new object?[] { null, false, false, null }));
		...
		/// <summary><see cref="FrooxEngine.Slot"/> child accessor <c>this[int]</c> (compiled name is usually <c>Item</c>).</summary>
		private static PropertyInfo? SlotIntIndexerProperty
		{
			get
			{
				...
				var slotType = AccessTools.TypeByName("FrooxEngine.Slot");
				if (slotType == null)
					return null;
		...
		internal static IEnumerable<object> EnumerateDescendantSlots(object root)
		{
			...
			while (stack.Count > 0 && steps++ < maxSteps)
			{
				var s = stack.Pop();
				yield return s;
				var countObj = ReadMemberValue(s, "ChildrenCount");
				if (countObj is not int cnt || cnt <= 0 || indexer == null)
					continue;
				for (var i = 0; i < cnt; i++)
				{
					var idx = i;
					var child = SafeRead(() => indexer.GetValue(s, new object[] { idx }));
					if (child != null)
						stack.Push(child);
				}
			}
